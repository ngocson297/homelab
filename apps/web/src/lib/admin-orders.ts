...
export type AdminOrderDetail = { orderCode: string; status: OrderStatus; statusLabel: string; version: number; contact: { name: string; phone: string }; appointment: { scheduledDate: string; timeSlot: string; province: string; district: string; ward: string; addressLine: string; note: string | null; status: string }; items: { testCode: string; testName: string; specimenType: string; price: string }[]; subtotal: string; collectionFee: string; totalAmount: string; timeline: { status: OrderStatus; title: string; description: string | null; occurredAt: string }[]; createdAt: string; updatedAt: string };
